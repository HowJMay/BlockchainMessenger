...
class Blockchain(object):
    ...
    def register_node(self, address):
        ...
        if parsed_url.netloc:
            self.nodes_set.add(parsed_url.netloc)
        elif parsed_url.path:
            # Accepts an URL without scheme like '192.168.0.5:5000'.
            self.nodes_set.add(parsed_url.path)
        else:
            raise ValueError('Invalid URL')
    def register_account(self, account):
        """
        Add a new account to the list of account
        """
        if not (account in self.accounts_set):
    ...
    def register_nodes_request(self):
        ...
        response = {
            ...
            "total_nodes" : list(self.nodes_set),
        }

        return jsonify(response), 201
    def add_account_request(self):
        values = request.get_json()

        new_accounts = values.get("new_accounts") 
        nodes = values.get("nodes")

        if nodes == None or new_accounts == None:
            return "Error: Please supply a valid list of noeds", 400
        
        for account, node in zip(new_accounts, nodes):
